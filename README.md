...
9. Now we want to combine all these individual text files into one large text files. This is useful for when you want to use the text to be searchable or in an LLM project. Run the "combine" script.
    ```python3 combine.py```
10. Now you should have one large text file. Now let's clean it up. This removes text such as the table of contents and miscellaneous navigation text like "Login here" that was scraped from every page. Removing these elements will improve the search quality of your project. Run the "cleanup" script.
    ```python3 cleanup.py```
